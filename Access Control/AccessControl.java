...
/**
 * The class AccessControl is an important part of this access control system. It makes a new AccessControl object, and 
 * it reports whether a given user name/password pair matches any user in the users ArrayList. It allow the current use 
 * log out their account, and update their passports. This class also authorize the admin add or remove users and take or give 
 * administration to one of the user.
 * 
 * AccessControl: Any non-static field will be initialized here. It also checks whether each class variable has been 
 * initialized and, if not, initialize them. 
 * 
 * isValidLogin: This method returns true if the user name/password pair matches any user in the users ArrayList and 
 * false otherwise. It relies on the static users ArrayList, and its only job is to determine whether a user 
 * name/password pair is valid.
 * 
 * setCurrentUser: This method will not be used by any other method in AccessControl, but it will be very useful for 
 * writing test methods.
 * 
 * logout: This method is to log out the current user, and set the currentUser to null.
 * 
 * changePassword: This method allow users to update their passport. modify the currentUser’s password to be the given 
 * newPassword.
 *
 * addUser: There will be two addUser method. One of them is to create a new user with the default password and another 
 * one is to create a new user and specify their administration status.
 * 
 * removeUser: This method remove user from the array list.The user name should be unique.
 * 
 ...
 * 
 * loginScreen: It help user log in into the access control system. This method consists of a single while(true) loop. 
 * 
 * sessionScreen: This method is one of the driver method in this program. It consists mostly of a single while loop. It 
 * set the currentUser to the user object matching the user name parameter. The function of this method is to set the 
 * currentUser and allow them to changePassword or logout 
 */
public class AccessControl {
  ...
  //new users or when we reset a user's password.Not be able to modify within the constructor.
  
  //A no-parameter constructor
  public AccessControl() {
    users.add(new User("admin", "root", true));
    currentUser = null;
    //System.out.println(users.get(users.size()-1).getUsername() + users.get(users.size()-1).password);
  }
  ...
  public boolean addUser(String username) {
    if (currentUser == null || !currentUser.getIsAdmin()) 
      return false;
    // return false if the current user is not an admin or there isn't a current user
    for (int i = 0; i < users.size(); i++)
      if (users.get(i).getUsername().equals(username))
        return false;
   ...
   * This method is one of the driver methods of this program
   */
  ...
  public void sessionScreen(String username, Scanner userInputScanner) {
    do {
      ...
      if (cmd.length == 3) {
        boolean isAdmin;
        if (cmd[2].equals("true")) isAdmin = true;
        else isAdmin = false;
        addUser(cmd[1], isAdmin);
      }
      else if (cmd[0].equals("logout")) logout();
      else if (cmd[0].equals("newpw")) changePassword(cmd[1]);
      else if (cmd[0].equals("adduser")) addUser(cmd[1]);
      else if (cmd[0].equals("rmuser")) removeUser(cmd[1]);
      else if (cmd[0].equals("giveadmin")) giveAdmin(cmd[1]);
      else if (cmd[0].equals("rmadmin")) takeAdmin(cmd[1]);
      else if (cmd[0].equals("resetpw")) resetPassword(cmd[1]);
