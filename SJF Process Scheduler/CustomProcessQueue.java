...
/**
 * The CustomProcessQueue class represents an array-based min heap containing processes.
 * This class implement WaitingQueueADT interface of CustomProcesses.
 */
public class CustomProcessQueue implements WaitingQueueADT<CustomProcess> {
  ...
  /**
   * This constructor creates a new instance of CustomProcessQueue with an empty array.
   * Initial capacity of the array is given in the final field. Initial size is 0.
   */
  ...
  private void minHeapPercolateUp(int index) { 
    while (index > 1) { // percolate up the last element, if it has higher priority than parent
      // swap it with its parent, then do it again until its parent has higher priority
  ...
  private void minHeapPercolateDown(int index) { 
    ...
    // percolate down the first element, swap it with the child with the higest priority
