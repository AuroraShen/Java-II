...
/**
 * This class creates nodes in a linked list. Each node has two fields: 1. primitive type integer
 * number to store value; 2. GameNode type object next to store the reference of next node.
 * The class has following methods and functions in turn:
 * @constructor GameNode: creates a new node and randomly assign a integer between 1-9 to the 
 *   number field.
 * @method getNumber: Accessor for the number field. Return the number of this node.
 * @method getNext: Accessor for the next field. Return the reference of the next node.
...
public class GameNode {
  ...
  /**
   * Calling on this method changes the number and next fields. The new number for this node is
   * calculated by applying the provided operator to this node’s number (the first operand),
   * and the next node’s number (the second operand). The new next reference for this node should
   * be copied from the next reference of the following node in the list so that the following node
