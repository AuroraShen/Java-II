...
public class Sequence implements Iterable<Integer> {
  // set of constants that represent the sequence types supported by this sequence 
  // generator
  ...
  public Sequence(int[] command) {
    ...
    switch (sequenceType) {
      case ARITHMETIC: // call the constructor of the ArithmeticSequenceGenerator 
    	// class to create an iterator over an arithmetic sequence with initial 
    	// element command[1], common difference command[2], and size stored at command[3]
      ...
      case FIBONACCI:
        ...
        break;
      case DIGIT_PRODUCT:
    	DigitProductSequenceGenerator interator = new DigitProductSequenceGenerator(command[1],
    			command[2]);
        sequenceIterator = interator.getIterator();
        break;
    }
  }
  ...
  public String toString() {
    ...
    // Use a for-each loop to traverse the sequence and add the different numbers of the 
    // sequence to its string representation, separated by a single space
    for (Integer i : this)
      seq += i + " ";
    return seq;
  }
  ...
  /**
   * The iterator method is defended in the Iterable<Integer> interface
   ...
   * @return true if the userCommand contains a syntax error with respect 
   * to the program specification, false otherwise
   */
   ...
   * Students do not modify any line of code within this driver method
   * @param args
   */
