...
public class Sett {
   ...
   * You must implement the naive insert algorithm discussed in zybooks and lecture: which only
   ...
   *         settle the badger with size {size}, as there is already a badger with the same size
   *         in this sett", where {size} needs to be replaced with the specified size parameter.
   */
   ...
   *         specified size. The message within this exception must read "WARNING: failed to find
   *         a badger with size {size} in the sett", where {size} needs to be replaced with the
   ...
   * 
   * @param current - The current Badger that is the root of a (sub) tree that we are searching
   *        for a Badger with the specified size within.
   * @param size - The size of the Badger object to search for and return.
   * @return The Badger found with the specified size.
   * @throws java.util.NoSuchElementException - When there is no Badger in this Sett with the
   *         specified size. The message within this exception must read "WARNING: failed to find
  ...
  private Badger findHelper(Badger current, int size) throws java.util.NoSuchElementException {
    if (current == null) // base case, end recursive, throw not found exception
      throw new NoSuchElementException("WARNING: failed to find a badger with size " 
          + size + " in the sett");
   ...
   * 
   * @param current - The current Badger that is the root of a (sub) tree that we are counting
   *        the number of Badgers within.
  ...
  private int getHeightHelper(Badger current) {
    if (current == null) // base case, end recursive
      return 0;
    // use max function to choose the larger height from left subtree or right subtree
