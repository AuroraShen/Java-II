...
/**
 * This class extends PApplet class. It makes the JunglePark class to be a PApplet 
 * which inherits its predefined properties and behaviors. Note that PApplet is a 
 * specific type that represents the Processing window for a graphic application or 
 * program. It provides good ways to create a new window and draw graphics onto it.
 * 
 * @author Ruoxi Shen
 */
public class JunglePark extends PApplet {
  private PImage backgroundImage; // PImage object that represents the background image
  protected ArrayList<ParkGUI> listGUI; // ArrayList storing the current graphical 
  // objects(animals present in the Jungle Park and buttons)
  protected AddAnimalButton tigerButton;
  protected AddAnimalButton deerButton;
  protected ClearButton clearButton;

  /**
   * CallBack method Defines initial environment properties such as screen size and 
   * to load background images and fonts as the program starts Initializes the 
   * backgroundImage and listGUI instance fields.
   */
  @Override
  public void setup() {
    this.getSurface().setTitle("Jungle Park"); // Displays text in the title of the 
    // display window
    this.textAlign(PApplet.CENTER, PApplet.CENTER); // Sets the current alignment 
    //for drawing text to CENTER
    this.imageMode(PApplet.CENTER); // Sets the location from which images are 
    // drawn to CENTER
    this.rectMode(PApplet.CORNERS); // Sets the location from which rectangles 
    // are drawn. rectMode(CORNERS) interprets the first two parameters of 
    // rect() method as the location of one corner, and the third and fourth 
    ...
    listGUI = new ArrayList<ParkGUI>(); // create the listGUI ArrayList that 
    // would store all the graphic objects (animals and buttons) that would be drawn on 
    // the display window
